...
public class LengthToMorseProcessor implements ISoundLengthReceiver {
    ...
    public void setSoundLenght(int lenght, boolean isSound) {
        ...
        if (state == State.FINDING_TICK_LENGTH) {
            if (!isSound) return;
            calculateTickLength(lenght);
        } else {
            if (isSound) {
                if (Utils.isApproxEqual(lenght, morseTickLength)) {
                    for (IMorseReceiver rec : receivers) {
                        rec.setMorseChar(MorseCharacter.DIT);
                    }
                    morseTickLength = Utils.average(lenght, morseTickLength);
                } else if (Utils.isApproxEqual(lenght, morseTickLength * MorseCharacter.DAH.getLenght())) {
                    for (IMorseReceiver rec : receivers) {
                        rec.setMorseChar(MorseCharacter.DAH);
                    }
                    morseTickLength = Utils.average(lenght, morseTickLength * MorseCharacter.DAH.getLenght()) / MorseCharacter.DAH.getLenght();
                }
            } else {
    ...
    public void calculateTickLength(int lenght) {
